...
DB.transaction(function (exe) {
	...
	*/
	exe.executeSql("CREATE TABLE IF NOT EXISTS cita_detalle (id INTEGER PRIMARY KEY, id_cita INTEGER, id_user_transportador integer,id_user_operario integer, peso_entrada float, peso_salida float,peso_neto float,autorizacion VARCHAR, estado VARCHAR,fecha_registro VARCHAR)");
...
function insertUser(){
	DB.transaction(function (exe) {
	    ...
	    exe.executeSql("insert into user (id_rol, email, password) values(3,'puerta@klever.com','12345')");
	    exe.executeSql("insert into user (id_rol, email, password, placa) values(1,'transporte@klever.com','12345', 'RCA-442')");
	    exe.executeSql("insert into user (id_rol, email, password, placa) values(1,'transporte2@klever.com','12345', 'SMG-332')");
	});   
}
